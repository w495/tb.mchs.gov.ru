<?xml version="1.0" encoding="utf-8"?>

<!DOCTYPE xsl:stylesheet [ <!ENTITY nbsp "&#160;"> ]>

<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
<!--
    Шаблон тела документа.
    Имеет вид:
    ~~~~~~~~~~~~~~~~~~~~~~
     $DateTime
    ~~~~~~~~~~~~~~~~~~~~~~     
     $Title
    ~~~~~~~~~~~~~~~~~~~~~~
     | $Pic |  $Text ...
         ...
    [~~~~~~~~~~~~~~~~~~~~~~
     Вложения:
        $Attaches
    ~~~~~~~~~~~~~~~~~~~~~~]
-->

<xsl:include href="../../shared/utils/erlangFormatDate.xsl" />
<xsl:include href="../../shared/utils/tipograf.xsl" />

<xsl:include href="question-body.xsl" />
<xsl:include href="ask-body.xsl" />
    
<xsl:template name="doc-body">
    <xsl:param name="Pic" />
    <xsl:param name="Title" />
    <xsl:param name="Text" />
    <xsl:param name="DateTime" />
    
    <xsl:param name="Attaches" />
    <xsl:param name="Experts" />
    <xsl:param name="Questions" />
    <xsl:param name="CurrentPath" />

    <xsl:param name="CurId" />    
    <xsl:param name="PrinterFlag" />

    <xsl:param name="GoBack" select="'#'"/>
    
    <!-- Заголовок конференции с датой -->
    <div id="_K_conference-body">
        <header>
            <div id="doc-body-datetime-holder">
                <time pubdate="pubdate">
                    <xsl:attribute name="datetime">
                        <xsl:call-template name="erlangFormatPubdate">
                            <xsl:with-param name="DateTime" select="$DateTime"/>
                            <xsl:with-param name="TimeZone" select="'MSD'"/>
                        </xsl:call-template>
                    </xsl:attribute>
                    <span id="doc-body-datetime" class="doc-body-border-bar">
                        <xsl:call-template name="erlangFormatDateTime">
                            <xsl:with-param name="DateTime" select="$DateTime"/>
                        </xsl:call-template>
                    </span>
                </time>
                <div class="breakline">&nbsp;</div>
            </div>        
            <h2 id="doc-body-header" class="second-header">
                <xsl:call-template name="tipograf">
                    <xsl:with-param  name="Input" select="$Title"/>
                </xsl:call-template>
            </h2>
            <div class="breakline">&nbsp;</div>
        </header>
        
        <!-- Описание конференции -->
        <section>
            <div id="doc-body-text">
                <xsl:if test="$Pic != '' ">
                    <!-- Картинки может и не быть -->
                    <div id="doc-body-picture" >
                        <img src="/{$Pic}" alt="{$Title}" />
                    </div>
                </xsl:if>            
                <xsl:call-template name="tipograf">
                    <xsl:with-param  name="Input" select="$Text"/>
                </xsl:call-template>
            </div>
            <xsl:if test="not($PrinterFlag)">
                <footer>
                    <xsl:if test="$Attaches/item">
                        <!-- Если нет вложений то вообще не показываем этот div-->
                        <section>
                            <div id="doc-body-attaches">
                                <h3 id="doc-body-attaches-header" class="second-header">Вложения:</h3>
                                <ul id="doc-body-attaches-list">
                                    <xsl:for-each select="$Attaches/item">
                                        <li class="doc-body-attaches-item">
                                            <!--
                                                <xsl:value-of select="attach_type_id" />
                                                <xsl:text:>&nbsp;</xsl:text:>
                                            -->
                                            
                                            <!-- возможно надо добавить картиночку -->
                                            
                                            <a href="/{url}"
                                               type="application/file"
                                               title="{alt}"
                                               alt="{name}" >
                                                <xsl:value-of select="name" />
                                            </a>
                                            <!--
                                                <xsl:text:>&nbsp;</xsl:text:>
                                                <xsl:value-of select="alt" />
                                            -->
                                        </li>
                                    </xsl:for-each>
                                </ul>
                            </div>
                        </section>
                    </xsl:if>
                </footer>
            </xsl:if>
        </section>
        
        <!-- Список экспертов -->
        <section>
            <!-- В идеале оно должно быть -->
            <div id="doc-body-experts">
                <h3>Экспертный совет</h3>
                <ul id="expert-list">
                    <xsl:for-each select="$Experts/item">
                    <li class="expert-item">
                        <xsl:if test="not($PrinterFlag)">
                            <div class="expert-photo">
                                <div class="expert-photo-image">
                                    <xsl:choose>
                                        <xsl:when test="(pic_url = '') or (pic_url = 'null')">
                                            <img src="/static/site-media/images/defaults/expert.png" height="92" title="{lastname}" alt="{lastname}" />
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <img src="/{pic_url}" alt="{lastname}" title="{lastname}" height="92" />
                                        </xsl:otherwise>                                    
                                    </xsl:choose>
                                </div>
                            </div>
                        </xsl:if>
                        <div class="expert-descr">
                            <div class="expert-names">
                                <span><xsl:value-of select="firstname" /></span>
                                <span>&nbsp;</span>
                                <span><xsl:value-of select="patronimic" /></span>
                                <span>&nbsp;</span>
                                <span><xsl:value-of select="lastname" /></span>
                                <span>&nbsp;</span>
                            </div>
                            <div class="expert-pos">
                                <span><xsl:value-of select="organization" /></span>
                                <span>&nbsp;</span>
                                <span><xsl:value-of select="position" /></span>
                            </div>
                        </div>
                    </li>
                    </xsl:for-each>
                </ul>
            </div>
        </section>
        
        <!-- Переход к вопросу -->
        <xsl:if test="not($PrinterFlag)">
            <xsl:if test="/data/meta/login != 'undefined'">
                <section>
                    <div id="ask-wrapper-wrapper">
                        <div id="ask-wrapper" >
                            <span id="ask" >
                                <xsl:text disable-output-escaping="yes">Задать вопрос →</xsl:text>
                            </span>
                        </div>
                        <div id="ask-body">
                            <!-- <h4>Задать вопрос</h4> -->
                            <xsl:call-template name="ask-body">
                                <xsl:with-param  name="CurId" select="$CurId"/>
                                <xsl:with-param  name="CurrentPath" select="$CurrentPath"/>
                            </xsl:call-template>
                        </div>
                    </div>
                </section>
            </xsl:if>
        </xsl:if>
        
        <!-- Список вопросов с ответами -->
        <section>
            <div id="doc-body-questions">
                <h3>
                    <xsl:text disable-output-escaping="yes">Вопросы</xsl:text>
                </h3>
                <div id="questions">
                    <ul id="question-list">
                        <xsl:for-each select="$Questions/item">
                        <li>
                            <xsl:call-template name="question-body">
                                <xsl:with-param  name="Question" select="."/>
                                <xsl:with-param  name="QuestionList" select="$Questions"/>
                                <xsl:with-param  name="CurrentPath" select="$CurrentPath"/>
                                <xsl:with-param  name="PrinterFlag" select="$PrinterFlag"/>
                            </xsl:call-template>
                        </li>
                        </xsl:for-each>
                    </ul>
                </div>
            </div>
        </section>
        
        <xsl:if test="not($PrinterFlag)">
            <footer>
                <div id="doc-body-goback-holder" >
                    <div class="breakline">&nbsp;</div>
                    <span id="doc-body-goback" class="doc-body-border-bar relief">
                        <a href="{/data/meta/self-parent-path}"  >
                            <xsl:text>вернуться</xsl:text>
                        </a>
                    </span>
                </div>
            </footer>
        </xsl:if>
        
        
        
    </div>
</xsl:template>

</xsl:stylesheet>
